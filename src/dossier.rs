...
pub struct DossierFacts {
    ...
    /// Author's other PRs open at scoring time. Agent fleets run many at
    /// once; measured concurrency for agent PRs is near 80 percent.
    pub concurrent_open: u64,
...
impl DossierFacts {
    ...
    pub fn rules(&self) -> Vec<Fire> {
        ...
        // Many simultaneously open PRs: the fleet signature.
        if self.concurrent_open >= 3 {
            out.push(Fire::new(
                "DOSSIER_CONCURRENT_OPEN",
                ((self.concurrent_open - 2) as f64 / 8.0).min(1.0),
            ));
        }
...
pub fn parse_dossier(login: &str, response: &Value, now: DateTime<Utc>) -> DossierFacts {
    ...
    if let Some(nodes) = user["pullRequests"]["nodes"].as_array() {
        for pr in nodes {
            // Only history that existed at scoring time counts. Live, every
            // node is in the past; in offline replay the snapshot is newer
            // than the scored PR and later nodes would leak the outcome.
...
mod tests {
    ...
    #[test]
    fn concurrent_open_prs_fire_above_two() {
        let mut facts = DossierFacts {
            concurrent_open: 2,
            has_bio: true,
            ..Default::default()
        ...
        let v = facts
            .rules()
            .iter()
            .find(|f| f.rule == "DOSSIER_CONCURRENT_OPEN")
            .unwrap()
            .value;
    ...
    #[test]
    fn claude_session_trailer_is_a_marker() {
        let (_, trailer, _) = scan_markers(
            &[],
            &["fix: thing\n\nClaude-Session: https://claude.ai/code/x".into()],
            "",
        );
        assert!(trailer);
    }
    ...
    fn parse_dossier_full_fixture() {
                ...
                "pullRequests": { "nodes": [
                    {
                        ...
                        "comments": { "nodes": [
                            { "author": { "login": "maintainer" }, "createdAt": "2025-01-02T00:00:00Z" }
                        ] },
                    ...
                    },
                    {
                        ...
                        "reviews": { "totalCount": 1 },
                        "comments": { "nodes": [
                            { "author": { "login": "maintainer" }, "createdAt": "2025-02-02T00:00:00Z" },
                            { "author": { "login": "ghost" }, "createdAt": "2025-02-03T00:00:00Z" }
                        ] },
                    ...
                    },
                    {
                        ...
                        "comments": { "nodes": [] },
                        "repository": { "nameWithOwner": "later/repo", "primaryLanguage": { "name": "Rust" } }
                    }
                ] }
            } }
        });
        ...
        // The third node postdates the scoring time and is excluded: replay
        // must not read history that did not exist yet.
        assert_eq!(facts.prior.len(), 2);
        assert!(facts.prior.iter().all(|p| p.repo_key != "later/repo"));
